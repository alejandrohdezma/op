...
```mermaid
---
title: "Triada CIA de la seguridad"
---
flowchart TB
    SEC["Seguridad en redes"]
    CONF["Confidencialidad\nSolo usuarios autorizados\nacceden a la informacion"]
    INTEG["Integridad\nLos datos no pueden ser\nalterados sin autorizacion"]
    DISP["Disponibilidad\nLos sistemas estan accesibles\ncuando se necesitan"]
...
sequenceDiagram
    ...
    participant S as Servidor<br>(tabla de conexiones)
    participant U as Usuario legitimo

    A->>S: SYN (peticion 1)
    S->>A: SYN-ACK
    Note right of S: Entrada en tabla de<br>conexiones semiabiertas

    A->>S: SYN (peticion 2)
    S->>A: SYN-ACK
    Note right of S: Otra entrada en tabla

    A->>S: SYN (peticion N...)
    S->>A: SYN-ACK
    Note right of S: Tabla llenandose...<br>El atacante NUNCA<br>envia el ACK final

    Note over S: La tabla de conexiones<br>semiestablecidas se agota

    U->>S: SYN (conexion legitima)
    S--xU: Conexion RECHAZADA
    Note over U,S: Denegacion de servicio:<br>el servidor no puede<br>admitir conexiones legitimas
```
...
```mermaid
---
title: "IDS (pasivo) vs IPS (en linea)"
---
flowchart LR
    subgraph IDS_SECTION ["Despliegue IDS (pasivo)"]
        direction LR
        S1_SRC["Origen del\ntrafico"] --> S1_SW["Switch\n(puerto SPAN)"]
        S1_SW --> S1_DST["Destino del\ntrafico"]
        S1_SW -. "Copia del trafico\n(puerto espejo)" .-> S1_IDS["IDS\n(solo detecta\ny alerta)"]
    end

    subgraph IPS_SECTION ["Despliegue IPS (en linea)"]
        direction LR
        S2_SRC["Origen del\ntrafico"] --> S2_IPS["IPS\n(detecta, alerta\ny bloquea)"]
        S2_IPS --> S2_DST["Destino del\ntrafico"]
    end
```
...
```mermaid
---
title: "Flujos de trafico permitidos en arquitectura DMZ"
---
flowchart LR
    INET["Internet"]
    DMZ["DMZ"]
    ...
    INET -- "PERMITIDO\n(servicios publicados)" --> DMZ
    DMZ -- "PERMITIDO\n(respuestas + DNS,\nactualizaciones)" --> INET
    LAN -- "PERMITIDO\n(administracion,\nusuarios internos)" --> DMZ
    LAN -- "PERMITIDO\n(con filtrado y proxy)" --> INET
    DMZ -. "BLOQUEADO\n(regla fundamental)" .-> LAN
    INET -. "BLOQUEADO" .-> LAN
```
...
```mermaid
---
title: "Arquitectura DMZ con un unico cortafuegos"
---
flowchart TB
    INET["Internet"]
    FW{{"FIREWALL\n(3 interfaces)"}}
    DMZ_ZONE["Servidores DMZ\nWeb, Mail"]
    LAN_ZONE["Usuarios internos\nLAN"]

    INET -- "Eth0 (WAN)" --- FW
    FW -- "Eth1 (DMZ)" --- DMZ_ZONE
    FW -- "Eth2 (LAN)" --- LAN_ZONE
```
...
```mermaid
---
title: "Arquitectura DMZ con doble cortafuegos"
---
flowchart TB
    INET["Internet"]
    FW_EXT{{"FIREWALL EXTERNO\n(solo permite trafico a DMZ)"}}
    DMZ_ZONE["DMZ\nWeb / Mail / DNS"]
    FW_INT{{"FIREWALL INTERNO\n(solo permite trafico DMZ a LAN\nbajo peticion explicita)"}}
    LAN_ZONE["LAN\nRed corporativa"]
...
```mermaid
---
title: "Evolucion de los cortafuegos"
---
timeline
    title Generaciones de cortafuegos
    section 1a Generacion (anos 80)
        Filtrado de paquetes : IP, puerto, protocolo
    section 2a Generacion (anos 90)
        Inspeccion de estado (stateful) : Contexto de conexion TCP
    section 3a Generacion (anos 90-2000)
        Proxy / Capa de aplicacion : Contenido HTTP, FTP, DNS
    section 4a Generacion (2000s)
        UTM / NGFW : DPI, antivirus, filtrado URL, IPS integrado
```
...
```mermaid
---
title: "Tipos de VPN por arquitectura de conexion"
---
flowchart TB
    VPN["VPN\n(Red Privada Virtual)"]

    subgraph RA ["Acceso Remoto (Remote Access)"]
        direction LR
        RA_USER["Usuario individual\n(teletrabajo,\nempleado en movilidad)"]
        RA_GW{{"Gateway VPN\ncorporativo"}}
        RA_LAN["Red corporativa"]
        RA_USER -- "Tunel cifrado\npor Internet" --> RA_GW
        RA_GW --- RA_LAN
    end

    subgraph SS ["Site-to-Site"]
        direction LR
        SS_NETA["Red Oficina A"]
        SS_FWA{{"FW/Router A"}}
        SS_FWB{{"FW/Router B"}}
        SS_NETB["Red Oficina B"]
        SS_NETA --- SS_FWA
        SS_FWA == "Tunel cifrado\npor Internet" === SS_FWB
        SS_FWB --- SS_NETB
    end
...
```mermaid
---
title: "Negociacion IKE en dos fases"
---
sequenceDiagram
    participant A as Peer A<br>(Iniciador)
    participant B as Peer B<br>(Respondedor)

    rect rgb(200, 220, 240)
        Note over A,B: Fase 1 - Establecer canal seguro (ISAKMP SA)
        A->>B: Propuesta de parametros<br>(algoritmos, grupo DH, autenticacion)
        B->>A: Aceptacion de parametros
        A->>B: Intercambio Diffie-Hellman
        B->>A: Intercambio Diffie-Hellman
        A->>B: Autenticacion de identidad
        B->>A: Autenticacion de identidad
        Note over A,B: Canal ISAKMP SA establecido
    end

    rect rgb(200, 240, 200)
        Note over A,B: Fase 2 - Negociar SA de IPSec
        A->>B: Propuesta IPSec SA<br>(ESP/AH, cifrado, duracion)
        B->>A: Aceptacion IPSec SA
        Note over A,B: Tunel IPSec operativo -<br>trafico cifrado con ESP
    end
```

```mermaid
---
title: "IPSec modo tunel (Site-to-Site)"
---
flowchart LR
    OA["Oficina A"]
    FWA{{"FW/Router A\n(Endpoint IPSec)"}}
    ...
    OA --- FWA
    FWA == "Tunel IPSec ESP\n(paquetes originales cifrados\ndentro de nuevos paquetes IP)" === FWB
    FWB --- OB
```
...
```mermaid
---
title: "Tipos de copia de seguridad a lo largo de una semana"
---
flowchart TB
    subgraph FULL ["Completa (Full)"]
        direction TB
        F_DESC["Copia TODOS los datos\ncada vez"]
        F_DOM["Domingo\n(Full)"]
        F_LUN["Lunes\n(Full)"]
        F_MAR["Martes\n(Full)"]
        F_MIE["Miercoles\n(Full)"]
        F_DOM --> F_LUN --> F_MAR --> F_MIE
        F_NOTE["Espacio: Alto\nRestauracion: Rapida\n(solo 1 copia)"]
    end

    subgraph INCR ["Incremental"]
        direction TB
        I_DESC["Copia solo cambios desde\nel ULTIMO backup"]
        I_DOM["Domingo\n(Full)"]
        I_LUN["Lunes\n(cambios desde Dom)"]
        I_MAR["Martes\n(cambios desde Lun)"]
        I_MIE["Miercoles\n(cambios desde Mar)"]
        I_DOM --> I_LUN --> I_MAR --> I_MIE
        I_NOTE["Espacio: Muy bajo\nRestauracion: Lenta\n(Full + todos los incrementales)"]
    end

    subgraph DIFF ["Diferencial"]
        direction TB
        D_DESC["Copia solo cambios desde\nel ULTIMO backup COMPLETO"]
        D_DOM["Domingo\n(Full)"]
        D_LUN["Lunes\n(cambios desde Dom)"]
        D_MAR["Martes\n(cambios desde Dom)"]
        D_MIE["Miercoles\n(cambios desde Dom)"]
        ...
        D_DOM --> D_MAR
        D_DOM --> D_MIE
        D_NOTE["Espacio: Medio\nRestauracion: Media\n(Full + ultimo diferencial)"]
    end
```
