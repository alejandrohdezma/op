...
```mermaid
flowchart TB
    F1["1. Identificar activos\ny su valor"]
    F2["2. Identificar amenazas\ny vulnerabilidades"]
    F3["3. Calcular el riesgo\n(probabilidad x impacto)"]
    F4{"4. Seleccionar\ncontramedidas"}
...
```mermaid
flowchart TB
    subgraph DIM["Dimensiones CITAD"]
        direction LR
        C["[C]\nConfidencialidad"]
        I["[I]\nIntegridad"]
        T["[T]\nTrazabilidad"]
    ...
    subgraph NIV["Niveles de seguridad por dimensión"]
        direction LR
        NB["BAJO\n(medidas básicas)"]
        NM["MEDIO\n(medidas reforzadas)"]
    ...
    DIM -->|cada dimensión se\nevalúa en un nivel| NIV

    MAX{"El nivel MÁS ALTO\nentre todas las\ndimensiones determina\nla categoría"}

    NIV --> MAX

    MAX -->|todas BAJO| CB["Categoría BÁSICA\n(impacto bajo)"]
    MAX -->|alguna MEDIO| CM["Categoría MEDIA\n(impacto significativo)"]
    MAX -->|alguna ALTO| CA["Categoría ALTA\n(impacto grave)"]
```
...
```mermaid
flowchart LR
    P["Planificar\n(Plan)\n\nDefinir alcance, política,\nanálisis de riesgos,\nplan de tratamiento"]
    H["Hacer\n(Do)\n\nImplantar controles,\nformar al personal,\ngestionar operaciones"]
    V["Verificar\n(Check)\n\nAuditar, revisar indicadores,\nevaluar eficacia\nde controles"]
...
flowchart LR
    ...
    subgraph ID["1. Identificación"]
        IDD["El usuario declara\nsu identidad\n(nombre, DNI...)"]
    end

    subgraph AU["2. Autenticación"]
        AUD["El sistema verifica\nla identidad"]
        F1["Algo que sabes\n(contraseña, PIN)"]
        F2["Algo que tienes\n(tarjeta, token OTP)"]
        F3["Algo que eres\n(huella, iris)"]
        AUD --- F1
    ...
    subgraph AZ["3. Autorización"]
        AZD["El sistema determina\nqué puede hacer\nel usuario autenticado"]
    end
...
```mermaid
---
title: "Tipos de Copia de Seguridad: Comparativa"
---
flowchart LR
    subgraph COMPLETO["Backup Completo"]
        direction LR
        CD1["Día 1\nCopia TOTAL"] --> CD2["Día 2\nCopia TOTAL"] --> CD3["Día 3\nCopia TOTAL"]
        RCC["Restaurar:\nsolo última\ncopia completa"]
    end

    subgraph INCREMENTAL["Backup Incremental"]
        direction LR
        ID1["Día 1\nCopia TOTAL"] --> ID2["Día 2\nCambios desde\nDía 1"] --> ID3["Día 3\nCambios desde\nDía 2"]
        RCI["Restaurar:\ncompleta + TODOS\nlos incrementales"]
    end

    subgraph DIFERENCIAL["Backup Diferencial"]
        direction LR
        DD1["Día 1\nCopia TOTAL"] --> DD2["Día 2\nCambios desde\nDía 1"] --> DD3["Día 3\nCambios desde\nDía 1"]
        RCD["Restaurar:\ncompleta + ÚLTIMO\ndiferencial"]
    end
```
...
flowchart TB
    ...
    REQ --> PDP{"Punto de decisión\nde políticas\n(nunca confiar,\nsiempre verificar)"}

    PDP --> MFA["MFA obligatorio\n(autenticación multifactor)"]
    MFA --> PRIV["Mínimo privilegio\n(solo permisos\nestrictamente necesarios)"]
    PRIV --> MICRO["Microsegmentación\n(limitar movimiento\nlateral en la red)"]
    MICRO --> MON["Monitoreo continuo\n(registrar y analizar\ntoda la actividad)"]

    MON --> OK["Acceso concedido\nal recurso solicitado"]
```
...
sequenceDiagram
    ...
    Note over E,R: Ambos comparten la misma clave secreta

    E->>E: Mensaje en claro +<br>clave compartida = Criptograma
    E->>R: Envío del criptograma por la red
    R->>R: Criptograma +<br>misma clave compartida = Mensaje en claro

    Note over E,R: Problema: la clave compartida debe<br>distribuirse de forma segura
```
...
sequenceDiagram
    ...
    Note over A,D: CIFRADO ASIMÉTRICO PARA CONFIDENCIALIDAD

    A->>A: Cifra el mensaje con la<br>clave PÚBLICA de David
    A->>D: Envío del criptograma
    D->>D: Descifra con su<br>clave PRIVADA

    Note over A,D: Solo David puede leer el mensaje<br>(CONFIDENCIALIDAD garantizada)
```
...
sequenceDiagram
    ...
    Note over D,A: FIRMA DIGITAL PARA AUTENTICIDAD

    D->>D: Firma/cifra el mensaje con<br>su clave PRIVADA
    D->>A: Envío del mensaje firmado
    A->>A: Descifra/verifica con la<br>clave PÚBLICA de David

    Note over D,A: Solo David pudo haber firmado<br>(AUTENTICIDAD y NO REPUDIO)
```
...
```mermaid
flowchart LR
    subgraph FIRMA["FIRMA (Emisor)"]
        direction LR
        M1["Mensaje\noriginal"] --> HF1["Función\nhash"]
        HF1 --> DG1["Resumen\n(hash)"]
        DG1 --> ENC["Cifrado con\nclave PRIVADA\ndel emisor"]
        ENC --> FD["Firma\ndigital"]
    end
    ...
    subgraph VERIFICACION["VERIFICACIÓN (Receptor)"]
        direction LR
        FD2["Firma\ndigital"] --> DEC["Descifrado con\nclave PÚBLICA\ndel emisor"]
        DEC --> HR["Hash\nrecibido"]
        M2["Mensaje\nrecibido"] --> HF2["Función\nhash"]
        HF2 --> HC["Hash\ncalculado"]
        HR --> CMP{"¿Hash recibido\n==\nHash calculado?"}
        HC --> CMP
        CMP -->|Sí| OK["FIRMA\nVÁLIDA"]
        CMP -->|No| KO["FIRMA\nINVÁLIDA"]
    end

    FIRMA --> ENVIO --> VERIFICACION
```
...
flowchart TB
    ...
    RA["Autoridad de Registro (RA)\nVerifica la identidad\ndel solicitante"]
    CA["Autoridad de Certificación (CA)\nEmite y gestiona\ncertificados digitales"]
    CERT["Certificado\ndigital X.509"]
    CRL["CRL\n(Lista de certificados\nrevocados)"]
    VA["Autoridad de Validación (VA)\nVerifica estado\ndel certificado"]
    OCSP["OCSP\n(Consulta en\ntiempo real)"]

    US -->|solicita| RA
    RA -->|verifica identidad| CA
    CA -->|emite| CERT
    CA -->|publica| CRL
    VA -->|consulta| CRL
    VA -->|responde vía| OCSP
    US -->|comprueba estado| VA
```
...
```mermaid
---
title: "Modos IPsec: Transporte vs Túnel"
---
flowchart TB
    subgraph TRANSPORTE["Modo Transporte"]
        direction LR
        TH["Cabecera IP\noriginal"] --> TE["Cabecera\nESP"] --> TP["Payload\ncifrado"] --> TT["Trailer\nESP"]
        TNOTE["Solo se cifra el payload;\nla cabecera IP original\nse mantiene"]
        TUSO["Uso: acceso remoto\nde administradores"]
    end

    subgraph TUNEL["Modo Túnel"]
        direction LR
        UH["Nueva\ncabecera IP"] --> UE["Cabecera\nESP"] --> UP["Cabecera IP original\n+ Payload\n(todo cifrado)"] --> UT["Trailer\nESP"]
        UNOTE["Se cifra el paquete completo\n(cabecera IP original + payload)\ny se añade nueva cabecera IP"]
        UUSO["Uso: VPN entre sedes\n(gateway a gateway)"]
    end
```
...
```mermaid
---
title: "VPN: Cifrado Combinado (Simétrico + Asimétrico)"
---
sequenceDiagram
    participant C as Cliente
    participant S as Servidor VPN

    Note over C,S: Combinación de cifrado simétrico y asimétrico

    C->>C: Genera clave de sesión<br>(AES, simétrica)
    C->>C: Cifra la clave de sesión con<br>clave PÚBLICA del servidor
    C->>S: Envía clave de sesión cifrada
    S->>S: Descifra la clave de sesión<br>con su clave PRIVADA

    Note over C,S: Ambos poseen la clave de sesión simétrica

    C->>S: Datos cifrados con clave de sesión (AES)
    S->>C: Datos cifrados con clave de sesión (AES)

    Note over C,S: Asimétrico para intercambio de clave<br>Simétrico para cifrado rápido de datos
```
...
```mermaid
---
title: "Handshake TLS/SSL"
---
sequenceDiagram
    participant A as Alice<br>(Cliente)
    participant B as Bob<br>(Servidor)

    Note over A,B: Handshake TLS/SSL

    A->>B: ClientHello<br>(versión SSL/TLS, preferencias cifrado, nonce RA)
    B->>A: ServerHello<br>(versión SSL/TLS, algoritmos elegidos, nonce RB)
    B->>A: Cadena de certificados X.509
    A->>A: Verifica el certificado<br>de Bob
    A->>B: Clave premaster cifrada<br>con clave pública de Bob
    Note over A,B: Ambos derivan la clave de sesión<br>a partir de: premaster + RA + RB
    A->>B: ChangeCipherSpec
    B->>A: ChangeCipherSpec
    A->>B: Comunicación cifrada simétrica<br>con clave de sesión
    B->>A: Comunicación cifrada simétrica<br>con clave de sesión
```
...
```mermaid
flowchart TB
    IN["Incidente detectado:\nInterrupción de comunicaciones"]
    P1["1. Informar a la Dirección\nde la situación"]
    P2{"2. Identificar causa:\n¿externa o interna?"}
    P3["3. Reportar al proveedor\nde servicio"]
    P4["4. Restablecer operación\ny determinar causas raíz;\ndefinir acciones preventivas"]
    P5["5. Anotar acciones y\nresultados en el registro\nde incidencias"]
    P6["6. Reportar el incidente\nal INCIBE"]
...
### 12.4 Organizaciones profesionales de TI

#### AITP (Association of Information Technology Professionals)

La **AITP** (*Association of Information Technology Professionals*) es una organización profesional dedicada a los profesionales de las tecnologías de la información. Su historia refleja la evolución del sector informático:

| Año | Nombre | Evento |
|---|---|---|
| **1949** | NMAA (National Machine Accountants Association) | Fundada en Chicago, EE. UU. |
| **1962** | DPMA (Data Processing Management Association) | Renombrada para reflejar la evolución hacia el procesamiento de datos |
| **1996** | **AITP** (Association of Information Technology Professionals) | Renombrada para abarcar las tecnologías de la información en sentido amplio |
| **2017** | Adquirida por **CompTIA** | CompTIA compró la asociación |
| **2019** | CompTIA IT Pro and Student Membership | CompTIA renombró la marca nacional, aunque muchos capítulos locales mantienen el nombre AITP |

**Objetivos de la AITP:**

- Promover el desarrollo profesional de los trabajadores de TI.
- Facilitar el acceso a programas de formación, certificaciones y recursos educativos.
- Fomentar el networking entre profesionales del sector.
- Establecer códigos éticos y estándares de buenas prácticas en el ámbito de las tecnologías de la información.
- Ofrecer competiciones y eventos para estudiantes universitarios de TI.

> **Dato de examen**: AITP = **Association of Information Technology Professionals**. Antes se llamaba DPMA (Data Processing Management Association). En 2017 fue adquirida por CompTIA.
