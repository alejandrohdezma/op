...
flowchart LR
    subgraph FIBRA["Estructura de la fibra óptica"]
        direction LR
        subgraph CUBIERTA["Cubierta exterior - coating/jacket"]
            subgraph REVESTIMIENTO["Revestimiento - cladding - índice n2"]
                NUCLEO["Núcleo\n(vidrio/plástico)\níndice n1\n(n1 > n2)"]
            end
        end
    end

    NOTA["La diferencia de índices\nde refracción (n1 > n2)\nprovoca la reflexión\ninterna total"]
    FIBRA ~~~ NOTA
```
...
flowchart TB
    AT["Área de trabajo\n(estaciones)"]
    CH["Cableado horizontal\n(máx. 90 m)"]
    TR["Armario de planta\n(TR - Telecommunications Room)"]
    CV["Cableado vertical\n/ backbone"]
    ER["Sala de equipos\n(ER - Equipment Room)"]
    BC["Backbone de campus"]
    IE["Instalación de entrada"]

    AT --> CH --> TR --> CV --> ER --> BC --> IE
```
...
```mermaid
flowchart TB
    subgraph OSI["Modelo OSI y equipos de red"]
        direction TB
...
```mermaid
---
title: "RDSI: Arquitectura y Puntos de Referencia"
---
flowchart LR
    TE2["TE2\n(terminal\nanalógico)"]
    TA["TA\n(adaptador\nde terminal)"]
...
flowchart TB
    ...
    OC["Orientadas a la conexión\n(Frame Relay, X.25, ATM)"]
    NOC["No orientadas a la conexión\n(Internet)"]
...
```mermaid
---
title: "Fases de la Conmutación de Circuitos"
---
sequenceDiagram
    participant TA as Terminal A
    participant RED as Equipos de<br>conmutación
    participant TB as Terminal B

    Note over TA,TB: Fase 1 - Establecimiento
    TA->>RED: Solicitud de conexión (dirección destino)
    RED->>TB: Establecimiento de camino físico

    Note over TA,TB: Fase 2 - Transmisión
    TA->>TB: Transmisión de datos (camino dedicado)
    TB->>TA: Transmisión de datos (camino dedicado)
    Note over TA,TB: El camino físico permanece reservado

    Note over TA,TB: Fase 3 - Liberación
    TA->>RED: Liberación de la conexión
    RED->>TB: Liberación del circuito
    Note over TA,TB: Los recursos del camino quedan libres
```
...
```mermaid
---
title: "Conmutación de Circuitos vs Paquetes"
---
sequenceDiagram
    box Conmutación de circuitos
    participant EA as Emisor A
    participant CA as Central
    participant RB as Receptor B
    end

    EA->>CA: Solicitud de conexión
    CA->>RB: Establecimiento de camino dedicado
    Note over EA,RB: Camino físico reservado durante toda la sesión
    EA->>RB: Datos (camino fijo)
    EA->>RB: Datos (camino fijo)
    EA->>CA: Liberación del circuito

    box Conmutación de paquetes
    participant EC as Emisor C
    participant N1 as Nodo 1
    participant N2 as Nodo 2
    participant RD as Receptor D
    end

    EC->>N1: Paquete 1
    EC->>N2: Paquete 2 (ruta alternativa)
    N1->>RD: Paquete 1
    EC->>N1: Paquete 3
    N2->>RD: Paquete 2
    N1->>RD: Paquete 3
    Note over EC,RD: Los paquetes pueden seguir rutas distintas y se reensamblan en destino
```
...
```mermaid
flowchart TB
    CT["CT - Central Terciaria / Nodal\n(solo 6 en España)"]
    CS["CS - Central Secundaria"]
    CP["CP - Central Primaria"]
    CL["CL - Central Local"]
    AB["Abonados"]
...
## 10. Redes de las Administraciones Públicas

### 10.1 Red SARA

La **Red SARA** (*Sistema de Aplicaciones y Redes para las Administraciones*) es el conjunto de infraestructuras de comunicaciones y servicios básicos que interconecta las redes de las **Administraciones Públicas españolas** e **instituciones europeas**, facilitando el intercambio seguro de información y el acceso a servicios compartidos.

**Definición y alcance:**

- Red SARA **no** se limita a un ámbito nacional ni autonómico: interconecta a **todas** las Administraciones Públicas (Administración General del Estado, Comunidades Autónomas, Entidades Locales) y también conecta con instituciones de la Unión Europea.
- Su objetivo principal es facilitar el intercambio seguro de información y el acceso a los servicios compartidos entre todas las Administraciones Públicas.
- Permite que cualquier unidad conectada pueda ofrecer servicios o utilizar los que pongan a disposición otras unidades de diferentes administraciones públicas españolas o europeas.

**Respaldo legislativo:**

| Norma | Artículo | Contenido |
|---|---|---|
| **Ley 11/2007**, de 22 de junio, de Acceso Electrónico de los Ciudadanos a los Servicios Públicos | **Artículo 43** | Establece la obligación de crear y mantener la Red de comunicaciones de las Administraciones Públicas españolas |
| **Real Decreto 4/2010**, de 8 de enero, por el que se regula el Esquema Nacional de Interoperabilidad | **Artículo 13** | Desarrolla la regulación de la Red SARA en el ámbito de la interoperabilidad entre AAPP |

**Características técnicas:**

- Ancho de banda de **10 Gbps** en Ministerios y Centros de Proceso de Datos.
- Ancho de banda de **100 Mbps** en cada Comunidad Autónoma.
- Tráfico cifrado para garantizar la confidencialidad de las comunicaciones.
- Sistema de Alerta Temprana para incidentes de seguridad, en colaboración con el **CCN-CERT**.

**Servicios que ofrece la Red SARA:**

- Interconexión de redes departamentales de la AGE.
- Conexión con las redes de las Comunidades Autónomas y Entidades Locales.
- Acceso a servicios horizontales comunes (firma electrónica, notificaciones, registro electrónico).
- Conexión con la red **sTESTA** (red transeuropea de servicios telemáticos entre administraciones de la UE).
- Servicio de resolución DNS interno para las AAPP.
- Servicio de correo electrónico seguro entre administraciones.
...
flowchart TB
    ...
    subgraph SERVICIOS["Servicios"]
        ...
        ALERTA["Sistema de Alerta\nTemprana\n(CCN-CERT)"]
    end

    SARA --> SERVICIOS
```

> **Dato de examen**: La Red SARA facilita el intercambio seguro de información y el acceso a servicios compartidos de **todas** las Administraciones Públicas (no solo las de ámbito nacional ni solo las autonómicas). Su respaldo legislativo se encuentra en el **artículo 43 de la Ley 11/2007** y en el **artículo 13 del RD 4/2010**.
