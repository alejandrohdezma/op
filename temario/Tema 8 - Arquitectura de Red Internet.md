...
| **E** | 240.0.0.0 — 255.255.255.255 | Experimental / indefinido | — |

El siguiente diagrama muestra visualmente la división de octetos en cada clase:

```mermaid
---
title: "Clases de direcciones IPv4: division de octetos"
---
flowchart TB
    subgraph claseA["Clase A — 0.0.0.0 a 127.255.255.255"]
        direction LR
        A1["Octeto 1\nID de Red\n8 bits"]
        A2["Octeto 2\nID de Estacion\n8 bits"]
        A3["Octeto 3\nID de Estacion\n8 bits"]
    ...
    subgraph claseB["Clase B — 128.0.0.0 a 191.255.255.255"]
        direction LR
        B1["Octeto 1\nID de Red\n8 bits"]
        B2["Octeto 2\nID de Red\n8 bits"]
        B3["Octeto 3\nID de Estacion\n8 bits"]
    ...
    subgraph claseC["Clase C — 192.0.0.0 a 223.255.255.255"]
        direction LR
        C1["Octeto 1\nID de Red\n8 bits"]
        C2["Octeto 2\nID de Red\n8 bits"]
        C3["Octeto 3\nID de Red\n8 bits"]
    ...
    subgraph claseD["Clase D — 224.0.0.0 a 239.255.255.255 — Multicast"]
        direction LR
        D1["32 bits — Direccion de grupo\nSin division red/estacion"]
    end

    subgraph claseE["Clase E — 240.0.0.0 a 255.255.255.255 — Experimental"]
        direction LR
        E1["32 bits — Reservado\nSin division red/estacion"]
    end

    claseA ~~~ claseB ~~~ claseC ~~~ claseD ~~~ claseE
```
...
La jerarquía de redes de Internet se organiza en tres niveles (Tiers) interconectados:
...
**IXP (Internet Exchange Point — Punto de Intercambio de Tráfico de Internet)**: Infraestructura física donde diferentes redes se conectan para intercambiar tráfico de forma directa sin pasar por terceras redes. Beneficios: reducción de costes, mejora de la latencia y mayor resiliencia.

```mermaid
---
title: "Jerarquia de redes: Modelo Tier"
---
flowchart TD
    subgraph tier1["Tier 1 — Backbone de Internet"]
        T1A["Tier 1\nAT&T"]
        T1B["Tier 1\nNTT"]
        T1C["Tier 1\nTelia"]
        T1A <--->|"Peering\ngratuito"| T1B
    ...
    subgraph tier2["Tier 2 — ISPs regionales/nacionales"]
        T2A["Tier 2\nISP Regional A"]
        T2B["Tier 2\nISP Regional B"]
    ...
    subgraph tier3["Tier 3 — ISPs de acceso"]
        T3A["Tier 3\nISP Local A"]
        T3B["Tier 3\nISP Local B"]
    end

    subgraph usuarios["Usuarios de Internet"]
        U1["Hogares"]
        U2["Empresas"]
    end
...
La siguiente vista muestra la conectividad desde el punto de vista del usuario final:

```mermaid
---
title: "Conectividad desde el punto de vista del usuario final"
---
flowchart LR
    Router["Router de\nInternet"]
    ...
    subgraph local["Red local"]
        AP["Access Point\nWi-Fi"]
        MovilWiFi["Dispositivos\nmoviles"]
        AP -->|"Wi-Fi"| MovilWiFi
    end

    subgraph servicios["Servicios en red publica"]
        SyncML["Servidor\nSyncML"]
        Correo["Servidor\nde correo"]
        Telefonia["Proveedor\nde telefonia"]
    end

    MovilGPRS["Dispositivos\nmoviles"]

    Router -->|"Cable"| AP
    Router -->|"Red publica"| SyncML
    Router -->|"Red publica"| Correo
    Router -->|"Red publica"| Telefonia

    Telefonia -->|"GPRS / 3G"| MovilGPRS
```
...
Estos modelos se denominan "Tier" refiriéndose a las capas de la aplicación (no a los ISPs), lo que puede generar confusión. En el contexto de desarrollo de aplicaciones:
...
```mermaid
---
title: "Arquitecturas de aplicaciones web — Modelos Tier"
---
flowchart LR
    subgraph tier1["1-Tier (Monolitica)"]
        direction TB
        M1["Interfaz +\nLogica de negocio +\nBase de datos"]
    end

    subgraph tier2["2-Tier (Cliente-Servidor)"]
        direction TB
        C2["Cliente\nUI + Logica"]
        S2[("Servidor\nBase de datos")]
        C2 <--> S2
...
---
flowchart TB
    subgraph capa4["Capa 4 — Aplicacion"]
        P4["HTTP, FTP, SMTP,\nDNS, Telnet, Gopher"]
    end

    subgraph capa3["Capa 3 — Transporte"]
        P3["TCP, UDP"]
    end

    subgraph capa2["Capa 2 — Interred (Internet)"]
        P2["IP, ICMP, ARP, RARP"]
    end

    subgraph capa1["Capa 1 — Interfaz de red y hardware"]
        P1["Ethernet, Token-Ring,\nFDDI, X.25, Wi-Fi, ATM"]
    end

    capa4 -->|"Encapsulacion"| capa3
    capa3 -->|"Encapsulacion"| capa2
    capa2 -->|"Encapsulacion"| capa1

    capa1 -->|"Desencapsulacion"| capa2
    capa2 -->|"Desencapsulacion"| capa3
    capa3 -->|"Desencapsulacion"| capa4
```
...
El siguiente diagrama ilustra cómo dos hosts (Host A en "dirección de red X" y Host B en "dirección de red Y") se comunican a través de un router. La comunicación fluye por capas:
...
```mermaid
---
title: "Enrutamiento IP entre dos hosts"
---
flowchart LR
    subgraph hostA["Host A — Direccion de red X"]
        direction TB
        AA["Aplicacion\n(mensaje)"]
        AT["TCP\n(segmentos)"]
        AI["IP\n(datagramas)"]
    ...
    subgraph hostB["Host B — Direccion de red Y"]
        direction BT
        BN["Interfaz de Red\nX.25 / RTC\n(tramas)"]
        BI["IP\n(datagramas)"]
        BT2["TCP\n(segmentos)"]
...
```mermaid
---
title: "Ciclo peticion-respuesta HTTP"
---
sequenceDiagram
    participant C as Cliente<br>(Navegador)
    participant S as Servidor<br>Web

    note over C,S: HTTP es sin estado (stateless):<br>cada peticion es independiente

    C->>S: Peticion HTTP (metodo, URI, version, headers, body)
    note right of S: El servidor procesa<br>la solicitud

    S->>C: Respuesta HTTP (version, codigo de estado, headers, body)

    note over C,S: Ejemplo
    C->>S: GET /index.html HTTP/1.1
    S->>C: HTTP/1.1 200 OK + cuerpo HTML
```
...
```mermaid
---
title: "Evolucion de las versiones de HTTP"
---
timeline
    title Evolucion de HTTP
    1991 : HTTP/0.9
         : Solo GET, solo HTML
         : Sin cabeceras
    1996 : HTTP/1.0
    ...
    2022 : HTTP/3 (RFC 9114)
         : Basado en QUIC (sobre UDP)
         : Resuelve head-of-line blocking
         : Handshake 1-RTT con TLS 1.3
```

#### HTTP/2 en detalle

Las características principales de HTTP/2:
...
```mermaid
---
title: "Pila de capas HTTPS"
---
flowchart TB
    HTTP["Capa de Aplicacion\nHTTP"]
    TLS["Capa de Seguridad\nTLS / SSL"]
...
```mermaid
---
title: "Evolucion de SSL/TLS"
---
timeline
    title Versiones de SSL y TLS
    1995 : SSL 2.0
         : Primera version publica
         : OBSOLETO y vulnerable
    1996 : SSL 3.0
         : Mejoras sobre SSL 2.0
         : OBSOLETO (ataque POODLE)
    1999 : TLS 1.0
         : Primer estandar IETF
         : OBSOLETO desde 2020 (RFC 8996)
    2006 : TLS 1.1
         : Mejoras de seguridad menores
         : OBSOLETO desde 2020 (RFC 8996)
    2008 : TLS 1.2 (RFC 5246)
         : Ampliamente usado
         : Seguro con configuracion correcta
         : VALIDO
    2018 : TLS 1.3 (RFC 8446)
         : Version actual y recomendada
         : Handshake 1-RTT, 0-RTT
         : Solo AEAD, PFS obligatorio
```
...
```mermaid
---
title: "Handshake TLS — Cifrado hibrido"
---
sequenceDiagram
    participant C as Cliente
    participant S as Servidor
    participant CA as Autoridad de<br>Certificacion (CA)

    rect rgb(240, 240, 255)
        note over C,S: Fase 1 — Handshake (criptografia ASIMETRICA)
        C->>S: ClientHello (versiones, cipher suites)
        S->>C: ServerHello + Certificado (clave publica)
        C->>CA: Verificacion del certificado
        CA->>C: Certificado valido
        C->>S: Intercambio de claves (ECDHE)
        S->>C: Intercambio de claves (ECDHE)
        note over C,S: Ambas partes derivan claves de sesion simetricas
    end

    rect rgb(240, 255, 240)
        note over C,S: Fase 2 — Transferencia de datos (criptografia SIMETRICA)
        C->>S: Datos cifrados con clave simetrica
        S->>C: Datos cifrados con clave simetrica
    end
```
...
```mermaid
---
title: "Flujo de correo electronico — SMTP, POP3 e IMAP"
---
flowchart LR
    Emisor["Cliente de correo\n(Emisor)"]
    ServEmisor["Servidor de correo\ndel emisor"]
    ServReceptor["Servidor de correo\ndel receptor"]
    ReceptorPOP["Cliente de correo\n(Receptor - POP3)"]
    ReceptorIMAP["Cliente de correo\n(Receptor - IMAP)"]

    Emisor -->|"SMTP\n(puerto 25/587)"| ServEmisor
    ServEmisor -->|"SMTP\n(relay entre servidores)"| ServReceptor
    ServReceptor -->|"POP3 (puerto 110/995)\nDescarga y elimina\ndel servidor"| ReceptorPOP
    ServReceptor -->|"IMAP (puerto 143/993)\nAcceso sincronizado\nconserva en servidor"| ReceptorIMAP
```
...
```mermaid
---
title: "Jerarquia DNS y flujo de resolucion"
---
flowchart TD
    Cliente["Cliente\n(Navegador)"]
    Resolver["Resolver DNS local\n(cache)"]
    Root["Root Servers\n(13 servidores raiz)"]
    TLD["Servidores TLD\n(.com, .es, .org)"]
    Auth["Nameservers\nautoritativos\n(ej: ns1.ejemplo.com)"]

    Cliente -->|"1. Consulta:\nwww.ejemplo.com?"| Resolver
    Resolver -->|"2. Consulta recursiva"| Root
    Root -->|"3. Responde con\nservidor TLD para .com"| Resolver
    Resolver -->|"4. Consulta al TLD"| TLD
    TLD -->|"5. Responde con\nnameserver autoritativo"| Resolver
    Resolver -->|"6. Consulta al\nnameserver autoritativo"| Auth
    Auth -->|"7. Responde con\nIP: 93.184.216.34"| Resolver
    Resolver -->|"8. Respuesta al cliente\n(almacena en cache)"| Cliente
```
...
```mermaid
---
title: "Richardson Maturity Model — Madurez REST"
---
flowchart BT
    L0["Nivel 0 — Pantano de POX\nUn unico endpoint\nUn unico metodo (POST)"]
    L1["Nivel 1 — Recursos\nURIs individuales por recurso\nEj: /usuarios, /usuarios/42"]
    L2["Nivel 2 — Verbos HTTP\nUso correcto de GET, POST,\nPUT, DELETE + codigos de estado"]
    L3["Nivel 3 — HATEOAS\nLos recursos incluyen hipervinculos\na acciones posibles"]
