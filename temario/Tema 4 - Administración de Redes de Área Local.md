...
```mermaid
---
title: "LDAP: Directory Information Tree (DIT)"
---
flowchart TB
    ROOT["DC=empresa,DC=es"]
...
```mermaid
---
title: Estructura Lógica del Disco
---
flowchart LR
    BS["Sector de\nArranque"]
    subgraph EP["Espacio Particionado"]
        P1["Partición 1\n(ej. NTFS)"]
        P2["Partición 2\n(ej. FAT32)"]
        P3["Partición 3\n(ej. exFAT)"]
    end
...
```mermaid
---
title: "Comparativa: DAS vs NAS vs SAN"
---
flowchart TB
    subgraph DAS["DAS (Direct Attached Storage)"]
        direction TB
        DS1["Servidor 1"] --- DD1["Discos"]
        DS2["Servidor 2"] --- DD2["Discos"]
        DS3["Servidor 3"] --- DD3["Discos"]
    end

    subgraph NAS_ARCH["NAS (Network Attached Storage)"]
        direction TB
        NL["LAN"]
        NS1["Servidor"] --- NL
        NC1["Cliente"] --- NL
        NL --- NNAS["Dispositivo NAS\n(NFS / CIFS-SMB)"]
    end

    subgraph SAN_ARCH["SAN (Storage Area Network)"]
        direction TB
        SL["LAN\n(tráfico normal)"]
        SS1["Servidor 1"] --- SL
        SS2["Servidor 2"] --- SL
        SS1 --- SSAN["Red SAN dedicada\n(Fibre Channel / iSCSI)"]
        SS2 --- SSAN
        SSAN --- SSTO["Almacenamiento\ncompartido"]
    end

    DAS ~~~ NAS_ARCH ~~~ SAN_ARCH
```
...
```mermaid
---
title: "RAID 5: Paridad Distribuida (4 discos)"
---
flowchart LR
    subgraph Disco1["Disco 1"]
        D1B1["A1"]
        D1B2["B2"]
        D1B3["C3"]
        D1B4["Dp"]
    ...
    Note["Ap, Bp, Cp, Dp = bloques de paridad\nLa paridad rota entre todos los discos\nTolera el fallo de 1 disco"]
```
...
```mermaid
---
title: "RAID 6: Doble Paridad Distribuida (5 discos)"
---
flowchart LR
    subgraph Disco1["Disco 1"]
        D1B1["A1"]
    ...
    Note["p, q = dos bloques de paridad por franja\nAmbas paridades rotan entre todos los discos\nTolera el fallo simultáneo de 2 discos"]
```
...
```mermaid
---
title: "Arquitectura SNMP: Componentes e Interacciones"
---
flowchart LR
    ...
    A1 -. "Trap / Notification\n(UDP 162)" .-> G
    A2 -. "Trap / Notification\n(UDP 162)" .-> G
    A3 -. "Trap / Notification\n(UDP 162)" .-> G
```
...
```mermaid
---
title: "DHCP: Proceso DORA"
---
sequenceDiagram
    participant C as Cliente DHCP
    participant S as Servidor DHCP

    Note over C: Sin dirección IP

    C ->> S: 1. DHCP Discover (broadcast)
    Note right of C: El cliente busca servidores\nDHCP en la red

    S ->> C: 2. DHCP Offer (unicast)
    Note left of S: El servidor ofrece una IP\n(ej. 192.168.1.100)

    C ->> S: 3. DHCP Request (broadcast)
    Note right of C: El cliente solicita\nla IP ofrecida

    S ->> C: 4. DHCP Acknowledge (unicast)
    Note left of S: El servidor confirma\nIP + Máscara + Gateway + DNS

    Note over C: IP asignada correctamente
```
...
sequenceDiagram
    participant C as Cliente DHCP
    participant L as Servidor DHCP<br>Legítimo
    participant R as Servidor DHCP<br>Rogue (no autorizado)

    Note over C: Solicita nueva IP

    C ->> L: 1. DHCP Discover (broadcast)
    C ->> R: 1. DHCP Discover (broadcast)

    R ->> C: 2. DHCP Offer (responde más rápido)
    L ->> C: 2. DHCP Offer (llega tarde)

    Note over C: Acepta la primera oferta\n(la del Rogue)

    C ->> R: 3. DHCP Request

    R ->> C: 4. DHCP Acknowledge\n(configuración incorrecta:\ngateway falso, DNS falso)

    Note over C: IP asignada con\nconfiguración maliciosa

    Note over C, R: Solo afecta a clientes que solicitan\nnueva concesión. Los que tienen\nconcesión activa no se ven afectados.

    Note over L, R: Protección: DHCP Snooping\nen switches gestionables
```
...
```mermaid
---
title: "WPA2-Enterprise: Flujo de Autenticación"
---
sequenceDiagram
    participant CL as Cliente WiFi
    participant AP as Controladora WiFi<br>(Access Point)
    participant RS as Servidor RADIUS<br>(NPS / FreeRADIUS)
    participant AD as Active Directory<br>/ eDirectory

    CL ->> AP: 1. Credenciales EAP/802.1X
    Note over CL, AP: Protocolo EAP/802.1X

    AP ->> RS: 2. Access-Request (RADIUS)
    Note over AP, RS: Protocolo RADIUS

    RS ->> AD: 3. Validar credenciales
    Note over RS, AD: LDAP / Kerberos

    AD -->> RS: 4. Credenciales válidas

    RS -->> AP: 5. Access-Accept (RADIUS)

    AP -->> CL: 6. Acceso concedido

    Note over CL: Conectado a la red WiFi
```
...
```mermaid
---
title: "TETRA: Estructura de Trama TDMA"
---
flowchart LR
    subgraph CARRIER["Portadora 25 kHz"]
        direction LR
        S1["Slot 1\nUsuario 1"]
        S2["Slot 2\nUsuario 2"]
        S3["Slot 3\nUsuario 3"]
    ...
    NOTE["TDMA: 4 usuarios comparten\n1 frecuencia de 25 kHz\ndividida en 4 ranuras de tiempo"]
```
