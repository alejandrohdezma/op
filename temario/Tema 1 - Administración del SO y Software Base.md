...
flowchart TB
    ...
    CPU <-->|"datos e instrucciones"| BUS
    BUS <-->|"datos e instrucciones"| MEM
    BUS <-->|"E/S"| IO
```
...
flowchart TB
    ...
    CPU <-->|"Bus de instrucciones"| MemInst
    CPU <-->|"Bus de datos"| MemData
    MemData <-->|"Bus de datos"| IO
```
...
stateDiagram-v2
    ...
    Ejecución --> Listo: Interrupción / fin de quantum
    Ejecución --> Bloqueado: Espera E/S o evento
    Bloqueado --> Listo: E/S completada o evento ocurrido
    Ejecución --> Terminado: Finalización
    Terminado --> [*]
```
...
```mermaid
---
title: Arquitectura de controladores de dispositivos
---
flowchart TB
...
flowchart LR
    ...
    subgraph Frontend ["Frontend (Análisis)"]
        C["Análisis Léxico"]
    ...
    subgraph Backend ["Backend (Síntesis)"]
        F["Código Intermedio"]
        G["Optimización"]
...
```mermaid
---
title: Pila gráfica en Linux
---
flowchart BT
    A["Hardware Gráfico (GPU)"]
    B["Kernel (DRM/KMS)"]
    C["Servidor de Pantalla\n(X.Org o Compositor Wayland)"]
    D["Compositor / Gestor de Ventanas\n(KWin / Mutter)"]
    E["Interfaz Gráfica (KDE Plasma / GNOME)"]
    F["Usuario"]
...
```mermaid
---
title: Hipervisores — Tipo 1 vs Tipo 2
---
flowchart TB
...
```mermaid
---
title: Ciclo de vida del software y líneas base
---
flowchart LR
    A["Planificación"] --> B["Requisitos"] --> C["Diseño"] --> D["Codificación"] --> E["Prueba"] --> F["Mantenimiento"]

    A -.-> A1["Línea base de planificación"]
    B -.-> B1["Línea base de requisitos"]
    C -.-> C1["Línea base de diseño"]
    D -.-> D1["Línea base de codificación"]
    E -.-> E1["Línea base de prueba"]
    F -.-> F1["Línea base de configuración del SW"]
```
...
```mermaid
---
title: Flujo de control de cambios (GCS)
---
flowchart TD
    A["Necesidad de cambio"] --> B["Petición de cambio"]
    B --> C["Evaluación"]
    C --> D{"Decisión del\nCCC / ACC"}

    D -->|Aprobado| E["Orden de Cambio\nde Ingeniería (OCI)"]
    D -->|Rechazado| J["Informar al\nsolicitante"]

    E --> F["Cola de cambios\npendientes"]
    F --> G["Implementación\ndel cambio"]
    G --> H["Auditoría de\nconfiguración"]
    H --> I["Informe de Estado (IEC)"]

    subgraph eval ["Aspectos evaluados"]
        T1["Aspectos técnicos"]
        T2["Efectos laterales"]
        T3["Impacto general"]
        T4["Costes"]
    end
