...
```mermaid
---
title: "Capas OSI con sus PDUs"
---
flowchart TB
    subgraph OSI["Modelo OSI - 7 Capas"]
        direction TB
        L7["Capa 7 - Aplicación\nPDU: Datos"]
        L6["Capa 6 - Presentación\nPDU: Datos"]
        L5["Capa 5 - Sesión\nPDU: Datos"]
    ...
    PROTO7["HTTP, FTP, SMTP,\nDNS, DHCP, SSH"] -.-> L7
    PROTO6["SSL/TLS, MIME,\nJPEG, MPEG"] -.-> L6
    PROTO5["RPC, NetBIOS,\nSIP"] -.-> L5
    PROTO4["TCP, UDP"] -.-> L4
    PROTO3["IP, ICMP, ARP,\nOSPF, BGP"] -.-> L3
    PROTO2["Ethernet, Wi-Fi,\nPPP, Bluetooth"] -.-> L2
    PROTO1["Cables, fibra óptica,\nondas de radio"] -.-> L1
```
...
```mermaid
---
title: "TCP vs UDP: Comparación visual del comportamiento"
---
flowchart LR
    subgraph TCP["TCP (orientado a conexión)"]
        direction TB
        T1["Cliente"]
        T2["Handshake 3 vías\nSYN -> SYN-ACK -> ACK"]
        T3["Transferencia de datos\ncon ACKs y retransmisión"]
        T4["Cierre 4 vías\nFIN -> ACK -> FIN -> ACK"]
        T5["Servidor"]

        T1 -->|"1. Establecer"| T2
        T2 -->|"2. Enviar datos"| T3
        T3 -->|"3. Cerrar"| T4
        T4 --> T5
    end

    subgraph UDP["UDP (sin conexión)"]
        direction TB
        U1["Cliente"]
        U2["Envía datagramas\ndirectamente\n(fire-and-forget)"]
        U3["Servidor"]
        ...
        U1 -->|"Sin handshake"| U2
        U2 -->|"Sin confirmación"| U3
    end

    COMP["TCP: Fiable, ordenado,\nmayor overhead\n(HTTP, FTP, SSH)\n\nUDP: Rápido, sin garantías,\nmenor overhead\n(DNS, DHCP, TFTP, VoIP)"]
```
...
```mermaid
---
title: "Encapsulación OSI: de Aplicación a Física"
---
flowchart TB
    subgraph Emisor["Emisor (de arriba a abajo)"]
        direction TB
        L7["Capa 7 - Aplicación\n\nDATOS"]
        L6["Capa 6 - Presentación\n\nH6 | DATOS"]
        L5["Capa 5 - Sesión\n\nH5 | H6 | DATOS"]
        L4["Capa 4 - Transporte\n\nH4 | H5 | H6 | DATOS\n= SEGMENTO"]
        L3["Capa 3 - Red\n\nH3 | H4 | H5 | H6 | DATOS\n= PAQUETE"]
        L2["Capa 2 - Enlace\n\nH2 | H3 | H4 | H5 | H6 | DATOS | FCS\n= TRAMA"]
        L1["Capa 1 - Física\n\n010110100101001...\n= BITS"]

        L7 -->|"Añade H6"| L6
        L6 -->|"Añade H5"| L5
        L5 -->|"Añade H4 (TCP/UDP)"| L4
        L4 -->|"Añade H3 (IP)"| L3
        L3 -->|"Añade H2 (MAC) + FCS"| L2
        L2 -->|"Convierte a señal"| L1
    end

    subgraph Receptor["Receptor (de abajo a arriba)"]
        direction TB
        R1["Capa 1 - Física\nRecibe bits"]
        R2["Capa 2 - Enlace\nElimina H2 y FCS"]
        R3["Capa 3 - Red\nElimina H3"]
        R4["Capa 4 - Transporte\nElimina H4"]
        R5["Capa 5 - Sesión\nElimina H5"]
        R6["Capa 6 - Presentación\nElimina H6"]
        R7["Capa 7 - Aplicación\nDATOS originales"]
...
```mermaid
---
title: "Comparativa: Modelo OSI (7 capas) vs Modelo TCP/IP (4 capas)"
---
flowchart LR
...
```mermaid
---
title: "Estructura de la cabecera IPv4"
---
flowchart TB
    subgraph Header["Cabecera IPv4 (20 bytes mínimo)"]
        direction TB
        subgraph Row1["Bits 0-31 (primera fila)"]
            direction LR
            F1["Versión\n(4 bits)"]
            F2["IHL\n(4 bits)"]
            F3["Tipo de\nServicio\n(8 bits)"]
        ...
        subgraph Row2["Bits 32-63 (segunda fila)"]
            direction LR
            F5["Identificador\n(16 bits)"]
            F6["Flags\n(3 bits)"]
            F7["Desplazamiento\nde Fragmento\n(13 bits)"]
        ...
        subgraph Row3["Bits 64-95 (tercera fila)"]
            direction LR
            F8["TTL\n(8 bits)"]
            F9["Protocolo\n(8 bits)\n1=ICMP 6=TCP\n17=UDP"]
            F10["Checksum de\nCabecera\n(16 bits)"]
        ...
        subgraph Row4["Bits 96-127 (cuarta fila)"]
            direction LR
            F11["Dirección IP de Origen (32 bits)"]
        end

        subgraph Row5["Bits 128-159 (quinta fila)"]
            direction LR
            F12["Dirección IP de Destino (32 bits)"]
        end

        subgraph Row6["Bits 160+ (opcional)"]
            direction LR
            F13["Opciones (si IHL > 5)"]
            F14["Relleno"]
            F13 --- F14
        end
...
```mermaid
---
title: "ARP: Proceso de resolución IP a MAC"
---
sequenceDiagram
    participant A as Host A<br>(192.168.1.10)
    participant RED as Red Local (LAN)
    participant B as Host B<br>(192.168.1.20)

    Note over A: Quiere comunicarse con 192.168.1.20
    A->>RED: ARP Request (BROADCAST)
    Note over A,RED: "¿Quién tiene la IP 192.168.1.20?<br>Dígame su MAC"

    RED->>B: (Todos los hosts reciben la petición)

    B->>A: ARP Reply (UNICAST)
    Note over B,A: "Soy yo. Mi MAC es<br>AA:BB:CC:DD:EE:FF"

    Note over A: Guarda la asociación en caché ARP
    Note over A: 192.168.1.20 -> AA:BB:CC:DD:EE:FF
```
...
```mermaid
---
title: "TCP: Three-Way Handshake (establecimiento de conexión)"
---
sequenceDiagram
    participant C as Cliente
    participant S as Servidor

    Note over C,S: Establecimiento de conexión (3 vías)
    C->>S: SYN (seq=x)
    Note right of C: Paso 1: Cliente solicita conexión
    S->>C: SYN-ACK (seq=y, ack=x+1)
    Note left of S: Paso 2: Servidor acepta y sincroniza
    C->>S: ACK (ack=y+1)
    Note right of C: Paso 3: Cliente confirma

    Note over C,S: CONEXIÓN ESTABLECIDA (estado ESTABLISHED)

    C->>S: Transferencia de datos
    S->>C: Transferencia de datos
```
...
```mermaid
---
title: "TCP: Cierre de conexión (4 vías / Four-Way Handshake)"
---
sequenceDiagram
    participant C as Cliente
    participant S as Servidor

    Note over C,S: Cierre de conexión (4 mensajes)
    C->>S: FIN
    Note right of C: Paso 1: Cliente inicia el cierre
    S->>C: ACK
    Note left of S: Paso 2: Servidor confirma recepción del FIN
    S->>C: FIN
    Note left of S: Paso 3: Servidor cierra su lado
    C->>S: ACK
    Note right of C: Paso 4: Cliente confirma el cierre
...
```mermaid
---
title: "Estructura de la cabecera UDP (8 bytes)"
---
flowchart TB
    subgraph UDPHeader["Cabecera UDP - 8 bytes total"]
        direction TB
        subgraph Row1["Bits 0-31"]
            direction LR
            SRC["Puerto Origen\n(16 bits)"]
            DST["Puerto Destino\n(16 bits)"]
            SRC --- DST
        end
        subgraph Row2["Bits 32-63"]
            direction LR
            LEN["Longitud\n(16 bits)"]
            CHK["Checksum\n(16 bits)"]
            LEN --- CHK
    ...
    subgraph Datos["Datos de la aplicación"]
        PAYLOAD["Payload\n(longitud variable)"]
    end
...
```mermaid
---
title: "DHCP: Proceso DORA (Discover-Offer-Request-Acknowledge)"
---
sequenceDiagram
    participant C as Cliente<br>(sin IP, UDP:68)
    participant S as Servidor DHCP<br>(UDP:67)

    Note over C,S: Proceso DORA

    C->>S: 1. DHCPDISCOVER (broadcast UDP)
    Note right of C: "¿Hay algún servidor DHCP?"

    S->>C: 2. DHCPOFFER (unicast/broadcast)
    Note left of S: "Te ofrezco IP: 192.168.1.50"

    C->>S: 3. DHCPREQUEST (broadcast UDP)
    Note right of C: "Acepto la IP 192.168.1.50"

    S->>C: 4. DHCPACK (unicast/broadcast)
    Note left of S: "Confirmado. IP: 192.168.1.50/24<br>Gateway, DNS, lease time..."

    Note over C,S: Cliente configurado con IP y parámetros de red
```
...
flowchart TD
    ...
    SLD_JUNTA[".junta.es"]
    SLD_EJEMPLO[".ejemplo.com"]

    HOST_INTRANET["intranet.junta.es"]
    HOST_WWW["www.ejemplo.com"]
    ...
    SLD_JUNTA --> HOST_INTRANET
    SLD_EJEMPLO --> HOST_WWW

    ROOT ~~~ NOTE1["13 servidores raíz\n(a-m).root-servers.net"]
    TLD_ES ~~~ NOTE2["TLD: dominios de\nprimer nivel"]
    SLD_JUNTA ~~~ NOTE3["Dominios de\nsegundo nivel"]
    HOST_INTRANET ~~~ NOTE4["Subdominios /\nhosts"]
```
...
```mermaid
---
title: "DNS: Resolución recursiva vs iterativa"
---
sequenceDiagram
    participant C as Cliente
    participant R as Resolver DNS<br>(recursivo)
    participant ROOT as Servidor Raíz
    participant TLD as Servidor TLD<br>(.com)
    participant AUTH as Servidor<br>Autoritativo<br>(ejemplo.com)

    Note over C,AUTH: Resolución RECURSIVA (típica)
    C->>R: Consulta: www.ejemplo.com?
    R->>ROOT: Consulta: www.ejemplo.com?
    ROOT-->>R: Referencia al servidor TLD (.com)
    R->>TLD: Consulta: www.ejemplo.com?
    TLD-->>R: Referencia al autoritativo (ejemplo.com)
    R->>AUTH: Consulta: www.ejemplo.com?
    AUTH-->>R: Respuesta: 93.184.216.34
    R-->>C: Respuesta final: 93.184.216.34

    Note over C,AUTH: Resolución ITERATIVA
    C->>ROOT: Consulta: www.ejemplo.com?
    ROOT-->>C: Referencia: pregunta al TLD (.com)
    C->>TLD: Consulta: www.ejemplo.com?
    TLD-->>C: Referencia: pregunta al autoritativo
    C->>AUTH: Consulta: www.ejemplo.com?
    AUTH-->>C: Respuesta: 93.184.216.34
```
...
```mermaid
---
title: "Clasificación de protocolos de enrutamiento"
---
flowchart TD
...
```mermaid
---
title: "OSPF: Arquitectura de áreas"
---
flowchart TD
    subgraph BACKBONE["Área 0 (Backbone)"]
        direction LR
        R1["Router\nBackbone"]
        R2["Router\nBackbone"]
        R3["Router\nBackbone"]
...
```mermaid
---
title: "Tipos de NAT: Estático, Dinámico y PAT"
---
flowchart LR
    subgraph STATIC["NAT Estático (1:1)"]
        direction LR
        S_PRIV["IP privada\n10.0.0.2"]
        S_PUB["IP pública\n203.0.113.10"]
        S_PRIV -->|"Mapeo fijo\n1 a 1"| S_PUB
    ...
    subgraph DYNAMIC["NAT Dinámico (N:M)"]
        direction LR
        subgraph D_PRIVS["IPs privadas"]
            D_P1["10.0.0.2"]
            D_P2["10.0.0.3"]
            D_P3["10.0.0.4"]
        end
        subgraph D_POOL["Pool de IPs públicas"]
            D_PUB1["203.0.113.10"]
            D_PUB2["203.0.113.11"]
        end
        D_P1 -->|"Asignación\ndinámica"| D_PUB1
    ...
    subgraph PAT["PAT / NAPT / Masquerade (N:1)"]
        direction LR
        subgraph P_PRIVS["IPs privadas"]
            P_P1["10.0.0.2:50001"]
            P_P2["10.0.0.3:50001"]
            P_P3["10.0.0.4:50001"]
        end
        P_ONE["Una sola IP pública\n203.0.113.1"]
        P_P1 -->|":60001"| P_ONE
        P_P2 -->|":60002"| P_ONE
        P_P3 -->|":60003"| P_ONE
    end
...
```mermaid
---
title: "PAT: Traducción de direcciones con puertos"
---
flowchart LR
