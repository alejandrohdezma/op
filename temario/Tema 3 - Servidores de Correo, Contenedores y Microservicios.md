...
```mermaid
---
title: "Flujo completo del correo electrónico: dos ciclos"
---
flowchart LR
    MUA_E["MUA Emisor\n(cliente de correo)"]
    SRV_E["Servidor SMTP\nEmisor"]
...
```mermaid
---
title: Flujo de una sesión SMTP
---
sequenceDiagram
    participant C as Cliente
    participant S as Servidor SMTP

    S->>C: 220 Servidor SMTP
    C->>S: HELO miequipo.midominio.com
    S->>C: 250 Hello, pleased to meet you
    C->>S: MAIL FROM: yo@midominio.com
    S->>C: 250 Ok
    C->>S: RCPT TO: destinatario@sudominio.com
    S->>C: 250 Ok
    C->>S: DATA
    S->>C: 354 End data with CRLF.CRLF
    Note over C,S: El cliente envía cabeceras y cuerpo del mensaje
    C->>S: Subject, From, To + cuerpo + punto final
...
El **remitente** (usuario con cliente Outlook) entrega su correo al **SMTP server emisor** (la "oficina de correos" del remitente), que lo transmite a través de Internet al **SMTP server receptor** (la "oficina de correos" del destinatario), desde donde los destinatarios recogen sus mensajes.

```mermaid
---
title: "Arquitectura de retransmisión SMTP: analogía de oficina de correos"
---
flowchart LR
    REM["Remitente\n(cliente Outlook)"]
    EMI["SMTP Server Emisor\n(oficina de correos\ndel remitente)"]
    INET(("Internet"))
    REC["SMTP Server Receptor\n(oficina de correos\ndel destinatario)"]
    DEST["Destinatarios"]

    REM -->|"entrega correo"| EMI
    EMI -->|"transmite\nvía SMTP"| INET
    INET -->|"transmite\nvía SMTP"| REC
    REC -->|"recogen\nmensajes"| DEST
```
...
```mermaid
---
title: "IMAP vs POP3: sincronización multidispositivo"
---
flowchart TB
    subgraph IMAP["IMAP — Sincronización centralizada"]
        direction TB
        SRV_I["Servidor de Correo\n(mensajes en servidor)"]
        PHONE["Smartphone"]
        PC["Ordenador\nde escritorio"]
        TABLET["Tablet"]

        SRV_I <-->|"sincronizado"| PHONE
        SRV_I <-->|"sincronizado"| PC
        SRV_I <-->|"sincronizado"| TABLET
    end

    subgraph POP3["POP3 — Descarga a un solo equipo"]
        direction TB
        SRV_P["Servidor de Correo\n(sin copia tras descarga)"]
        EQUIPO["Equipo único"]

        SRV_P -->|"descarga y\nelimina del servidor"| EQUIPO
    end
```
...
```mermaid
---
title: "Proceso de intercambio de correo electrónico: 5 pasos"
---
sequenceDiagram
    participant MUA_E as MUA Emisor
    participant MTA_E as MTA Emisor\n(servidor local)
    participant MTA_R as MTA Receptor\n(servidor destinatario)
    participant MUA_R as MUA Receptor

    MUA_E->>MTA_E: 1. Crea y envía el correo
    Note over MTA_E: 2. Almacena el mensaje\nen el servidor local
    MTA_E->>MTA_R: 3. Negociación SMTP y envío
    Note over MTA_R: 4. Corrobora la operación\ny deposita en el buzón
    MUA_R->>MTA_R: 5. Solicita correo (POP3/IMAP)
    MTA_R->>MUA_R: 5. Entrega el mensaje
```
...
```mermaid
---
title: "Cadena de autenticación: SPF, DKIM y DMARC"
---
sequenceDiagram
    participant EM as Servidor Emisor
    participant RE as Servidor Receptor
    participant DNS as DNS del dominio emisor

    EM->>RE: Envía correo electrónico
    Note over RE: Inicia verificaciones de autenticación

    RE->>DNS: Consulta registro SPF
    DNS->>RE: Lista de IPs autorizadas
    Note over RE: Verifica si IP del emisor está autorizada

    RE->>DNS: Consulta clave pública DKIM
    DNS->>RE: Clave pública del dominio
    Note over RE: Verifica firma criptográfica de la cabecera

    RE->>DNS: Consulta política DMARC
    DNS->>RE: Política (entregar / cuarentena / rechazar)
    Note over RE: Aplica política DMARC según resultados SPF y DKIM
```
...
```mermaid
---
title: "Triángulo SOA: SOAP, WSDL y UDDI"
---
flowchart TB
    UDDI["Registro UDDI\n(páginas amarillas)"]
    PROV["Proveedor\ndel Servicio"]
    ...
    PROV -->|"1. Publica WSDL"| UDDI
    CONS -->|"2. Descubre servicio"| UDDI
    UDDI -->|"3. Devuelve WSDL"| CONS
    CONS -->|"4. Invoca servicio\n(SOAP/XML)"| PROV
```
...
```mermaid
---
title: "ESB vs Punto a Punto: reducción de complejidad"
---
flowchart TB
    subgraph P2P["Sin ESB — Punto a Punto\n(45 conexiones para 10 apps)"]
        direction TB
        A1["App 1"] <--> A2["App 2"]
        A1 <--> A3["App 3"]
        A1 <--> A4["App 4"]
    ...
    subgraph CON_ESB["Con ESB — Hub centralizado\n(10 conexiones para 10 apps)"]
        direction TB
        B1["App 1"] <--> ESB["ESB\n(Bus de Servicios\nEmpresariales)"]
        B2["App 2"] <--> ESB
        B3["App 3"] <--> ESB
...
```mermaid
---
title: "Bus de eventos: comunicación asíncrona entre microservicios"
---
flowchart LR
    A["Microservicio A\n(publicador)"]
    BUS["Bus de Eventos"]
    ...
    A -->|"publica evento"| BUS
    BUS -->|"entrega evento"| B
    BUS -->|"entrega evento"| C
```
...
```mermaid
---
title: "Contenedores vs Máquinas Virtuales: arquitectura comparada"
---
flowchart TB
    subgraph VM["Máquinas Virtuales"]
        direction TB
        VM_APP1["App 1"] --> VM_BIN1["Bins/Libs"]
        VM_BIN1 --> VM_GOS1["Guest OS"]
        VM_APP2["App 2"] --> VM_BIN2["Bins/Libs"]
        VM_BIN2 --> VM_GOS2["Guest OS"]
        VM_APP3["App 3"] --> VM_BIN3["Bins/Libs"]
        VM_BIN3 --> VM_GOS3["Guest OS"]
        VM_GOS1 --> HYP["Hypervisor"]
        VM_GOS2 --> HYP
        VM_GOS3 --> HYP
        HYP --> VM_HOS["Host Operating System"]
        VM_HOS --> VM_INF["Infraestructura"]
    end

    subgraph CT["Contenedores"]
        direction TB
        CT_APP1["App 1"] --> CT_BIN1["Bins/Libs"]
        CT_APP2["App 2"] --> CT_BIN2["Bins/Libs"]
        CT_APP3["App 3"] --> CT_BIN3["Bins/Libs"]
        CT_BIN1 --> ENG["Container Engine\n(Docker)"]
        CT_BIN2 --> ENG
        CT_BIN3 --> ENG
        ENG --> CT_OS["Operating System\n(compartido)"]
        CT_OS --> CT_INF["Infraestructura"]
    end
```
...
```mermaid
---
title: "Docker: mecanismos del kernel Linux (libcontainer)"
---
flowchart TB
    DOCKER["Docker"]
    LIBCONT["libcontainer"]
    KERNEL["Linux Kernel"]
...
```mermaid
---
title: "Hipervisores: Tipo 1 (bare-metal) vs Tipo 2 (hosted)"
---
flowchart TB
    subgraph T1["Tipo 1 — Bare-metal / Nativo"]
        direction TB
        T1_VM1["VM 1\n(Guest OS + App)"]
        T1_VM2["VM 2\n(Guest OS + App)"]
        T1_VM3["VM 3\n(Guest OS + App)"]
        T1_VM1 --> T1_HYP["Hypervisor\n(ESXi, Hyper-V, XenServer)"]
        T1_VM2 --> T1_HYP
        T1_VM3 --> T1_HYP
        T1_HYP --> T1_HW["Hardware Físico"]
    end

    subgraph T2["Tipo 2 — Hosted / Alojado"]
        direction TB
        T2_VM1["VM 1\n(Guest OS + App)"]
        T2_VM2["VM 2\n(Guest OS + App)"]
        T2_VM3["VM 3\n(Guest OS + App)"]
        T2_VM1 --> T2_HYP["Hypervisor\n(VirtualBox, VMware\nWorkstation, Parallels)"]
        T2_VM2 --> T2_HYP
        T2_VM3 --> T2_HYP
        T2_HYP --> T2_HOS["Sistema Operativo\nAnfitrión (Host OS)"]
        T2_HOS --> T2_HW["Hardware Físico"]
    end
```
...
```mermaid
---
title: Arquitectura de un clúster Kubernetes
---
flowchart TB
    subgraph CP["Control Plane (nodo maestro)"]
        direction TB
        API["API Server"]
        SCHED["Scheduler"]
...
## 5. Conceptos Complementarios de Red y Seguridad

### 5.1 DNS (Domain Name System)

El **DNS** (*Domain Name System*, Sistema de Nombres de Dominio) es un sistema de nomenclatura **jerárquico y descentralizado** para dispositivos conectados a redes IP, como Internet o redes privadas.

**Función principal:**

La función más importante del DNS es la **resolución de nombres**: traducir nombres de dominio inteligibles para las personas (por ejemplo, `www.ejemplo.es`) en direcciones IP numéricas (por ejemplo, `93.184.216.34`) que las máquinas necesitan para comunicarse entre sí.

**Características técnicas:**

- Opera en el **puerto 53**, tanto sobre **UDP** (para consultas estándar) como sobre **TCP** (para transferencias de zona o respuestas de gran tamaño).
- Se organiza en una **jerarquía de niveles**:
  - **Servidores raíz**: la cúspide de la jerarquía. Existen 13 sistemas de servidores raíz lógicos, identificados con las letras A a M.
  - **Servidores TLD** (*Top-Level Domain*): responsables de los dominios de nivel superior (.com, .es, .org, .net).
  - **Servidores autoritativos**: contienen los registros de recursos para un dominio específico.
- Los principales **tipos de registros DNS** son:
  ...
  - **NS** (*Name Server*): identifica los servidores DNS autoritativos para el dominio.
  - **PTR**: resolución inversa (de IP a nombre de dominio).

**Proceso de resolución:**

1. El cliente envía una consulta **recursiva** a su servidor DNS local.
2. El servidor DNS local verifica su caché; si tiene la respuesta, la devuelve.
3. Si no está en caché, realiza consultas **iterativas** a los servidores raíz, TLD y autoritativos.
4. El servidor DNS local entrega la resolución al cliente y almacena el resultado en caché.

```mermaid
---
title: "Proceso de resolución DNS: consultas recursivas e iterativas"
---
sequenceDiagram
    participant CL as Cliente
    participant LOCAL as Servidor DNS Local
    participant ROOT as Servidor Raíz
    participant TLD as Servidor TLD
    participant AUTH as Servidor Autoritativo

    CL->>LOCAL: 1. Consulta recursiva
    Note over LOCAL: Verifica caché local

    alt Respuesta en caché
        LOCAL->>CL: Devuelve respuesta desde caché
    else No está en caché
        LOCAL->>ROOT: 2. Consulta iterativa
        ROOT->>LOCAL: Referencia al servidor TLD
        LOCAL->>TLD: 3. Consulta iterativa
        TLD->>LOCAL: Referencia al servidor autoritativo
        LOCAL->>AUTH: 4. Consulta iterativa
        AUTH->>LOCAL: Respuesta con la IP
        Note over LOCAL: Almacena resultado en caché
        LOCAL->>CL: Entrega la resolución al cliente
    end
```

> **Dato de examen**: DNS = **Domain Name System** (no "Domain Number System", ni "Domain Name Software"). Opera en el **puerto 53**.

### 5.2 Algoritmos hash criptográficos

Una **función hash criptográfica** es un algoritmo matemático que transforma un bloque de datos de longitud variable en una cadena de longitud fija denominada **hash**, **resumen** o **digest**. Las funciones hash son unidireccionales: a partir del hash no se puede reconstruir el mensaje original.
...
- **Resistente a preimagen**: dado un hash, no es factible encontrar el mensaje original.
- **Resistente a colisiones**: no es factible encontrar dos mensajes diferentes que produzcan el mismo hash.
- **Efecto avalancha**: un cambio mínimo en la entrada produce un hash completamente diferente.

#### MD5 (Message-Digest Algorithm 5)

**MD5** es un algoritmo de reducción criptográfico que produce un valor hash de **128 bits** (16 bytes), representado habitualmente como una cadena de **32 caracteres hexadecimales**.

| Característica | Detalle |
|---|---|
| **Diseñador** | Ronald Rivest (MIT) |
| **Año** | 1991 (especificado en 1992 como **RFC 1321**) |
| **Longitud del hash** | **128 bits** |
| **Predecesor** | MD4 |
| **Estado actual** | **Inseguro** para aplicaciones criptográficas; se han demostrado colisiones desde 2004 |

**Usos actuales de MD5** (no criptográficos):
- Verificación de integridad de archivos descargados (checksums).
- Generación de identificadores únicos en bases de datos.
- Detección de duplicados en sistemas de almacenamiento.
...
| **SHA-256** | 256 bits | Seguro; ampliamente utilizado (parte de SHA-2) |
| **SHA-384** | 384 bits | Seguro (parte de SHA-2) |
| **SHA-512** | 512 bits | Seguro (parte de SHA-2) |
| **SHA-3** | Variable (224-512 bits) | Estándar más reciente (basado en Keccak) |

> **Dato de examen**: MD5 genera un hash de **128 bits** y fue diseñado por **Ronald Rivest**. No es adecuado para aplicaciones de seguridad actuales por sus colisiones conocidas. No confundir MD5 con MD3 (que no es un estándar reconocido) ni con POP/POP3 (que son protocolos de correo).
