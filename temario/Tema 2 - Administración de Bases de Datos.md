...
```mermaid
---
title: Arquitectura ANSI-SPARC (tres niveles de abstracción)
---
flowchart TB
    EXTERNAL["Nivel Externo\n(Vistas de usuario)\nDistintas vistas para cada usuario/app"]
    CONCEPTUAL["Nivel Conceptual\n(Lógico)\nEntidades, relaciones, restricciones"]
    INTERNAL["Nivel Interno\n(Físico)\nBloques, índices, almacenamiento en disco"]

    EXTERNAL -->|"Independencia lógica"| CONCEPTUAL
    CONCEPTUAL -->|"Independencia física"| INTERNAL
```
...
#### Puertos por defecto de los principales SGBD

En el examen pueden preguntar el puerto TCP/IP por defecto de las tecnologías de bases de datos más comunes. Esta tabla incluye tanto sistemas relacionales como NoSQL:

| SGBD | Puerto por defecto | Protocolo |
|---|---|---|
...
La jerarquía de almacenamiento lógico de Oracle es la siguiente:

```mermaid
---
title: Jerarquía de almacenamiento lógico de Oracle
---
flowchart TB
    BD["Base de Datos"]
    TS["Tablespace\n(unidad lógica; SYSTEM = diccionario de datos)"]
    SEG["Segmento\n(conjunto de extensiones de un objeto: tabla, índice...)"]
...
```mermaid
---
title: Progresión de formas normales
---
flowchart LR
    ...
    FN1 -->|"Elimina dependencias parciales"| FN2
    FN2 -->|"Elimina dependencias transitivas"| FN3
    FN3 -->|"Todo determinante es superclave"| FNBC
```
...
En **DAS** el flujo va directamente de aplicación a disco; en **SAN** pasa por una red FC/GbE; en **NAS** la aplicación accede a un sistema de ficheros remoto a través de la red.

```mermaid
---
title: Comparativa DAS, SAN y NAS
---
flowchart LR
    subgraph DAS["DAS (Direct Attached Storage)"]
        direction TB
        APP1["Aplicación"]
        FS1["Sistema de Archivos\n(LOCAL)"]
        DISK1["Disco\n(SCSI/SATA/SAS)"]
        APP1 --> FS1 --> DISK1
    end

    subgraph SAN["SAN (Storage Area Network)"]
        direction TB
        APP2["Aplicación"]
        FS2["Sistema de Archivos\n(SERVER)"]
        NETSAN["Red SAN\n(FC/iSCSI)"]
        DISK2["Disco\n(REMOTE)"]
        APP2 --> FS2 --> NETSAN --> DISK2
    end
...
```mermaid
---
title: Estructura de una SAN (tres capas)
---
flowchart TB
    subgraph CapaHost["Capa Host"]
        Servidor1["Servidor 1"]
        Servidor2["Servidor 2"]
        ServidorN["Servidor N"]
    end

    subgraph CapaFibra["Capa de Fibra/Conexión"]
        Switch1["SAN Switch 1"]
        Switch2["SAN Switch 2"]
    end

    subgraph CapaAlmacenamiento["Capa de Almacenamiento"]
        Matriz["Matriz de Discos"]
        Cintas["Cintas"]
    end

    Servidor1 & Servidor2 & ServidorN --> Switch1 & Switch2
    Switch1 & Switch2 --> Matriz & Cintas
```
...
Los dispositivos de usuario se conectan a través de un **Connection Broker** al hipervisor, que asigna VMs a cada usuario.

```mermaid
---
title: Arquitectura VDI
---
flowchart LR
    subgraph Dispositivos["Dispositivos de usuario"]
        Portatil["Portátil"]
        Tablet["Tablet"]
    ...
    subgraph Escritorios["Escritorios virtuales"]
        VM1["VM 1"]
        VM2["VM 2"]
        VMN["VM N"]
    end
...
```mermaid
---
title: "Ejemplo semanal: restauración incremental vs diferencial"
---
flowchart LR
    subgraph incremental["Restauración Incremental (miércoles)"]
        full1["Completo Domingo"]
        incr1["Incr. Lunes"]
        incr2["Incr. Martes"]
        incr3["Incr. Miércoles"]
        full1 --> incr1
        incr1 --> incr2
        incr2 --> incr3
    end

    subgraph differential["Restauración Diferencial (miércoles)"]
        full2["Completo Domingo"]
        dif["Dif. Miércoles"]
        full2 --> dif
    end
```
...
```mermaid
---
title: RPO y RTO en una línea temporal
---
flowchart LR
    A["Último backup"]
    B["Desastre"]
    ...
    A -->|"RPO: Datos que se pueden perder"| B
    B -->|"RTO: Tiempo máximo de caída"| C
```
...
```mermaid
---
title: "Modelos de servicio cloud: responsabilidades"
---
flowchart TB
    subgraph IaaS["IaaS"]
        direction TB
        Dat1["Datos\n(Cliente)"]
        App1["Aplicación\n(Cliente)"]
        Mid1["Middleware/Runtime\n(Cliente)"]
        SO1["Sistema Operativo\n(Cliente)"]
        Infra1["Infraestructura\n(Proveedor)"]
        Dat1 --- App1
        App1 --- Mid1
        Mid1 --- SO1
        SO1 --- Infra1
    ...
    subgraph PaaS["PaaS"]
        ...
        Dat2["Datos\n(Cliente)"]
        App2["Aplicación\n(Cliente)"]
        Mid2["Middleware/Runtime\n(Proveedor)"]
        SO2["Sistema Operativo\n(Proveedor)"]
        Infra2["Infraestructura\n(Proveedor)"]
        Dat2 --- App2
        App2 --- Mid2
        Mid2 --- SO2
        SO2 --- Infra2
    ...
    subgraph SaaS["SaaS"]
        ...
        Dat3["Datos\n(Proveedor)"]
        App3["Aplicación\n(Proveedor)"]
        Mid3["Middleware/Runtime\n(Proveedor)"]
        SO3["Sistema Operativo\n(Proveedor)"]
        Infra3["Infraestructura\n(Proveedor)"]
        Dat3 --- App3
        App3 --- Mid3
        Mid3 --- SO3
        SO3 --- Infra3
...
## 13. Herramientas CASE y Frameworks de Desarrollo

### 13.1 Definición de CASE

**CASE** (*Computer Aided Software Engineering*, Ingeniería de Software Asistida por Computador) es el conjunto de herramientas y técnicas que permiten aplicar métodos automatizados al proceso de desarrollo de software, con el objetivo de incrementar la productividad del equipo de desarrollo y reducir los costes en tiempo y recursos.

**Características principales de las herramientas CASE:**

- Automatizan total o parcialmente actividades del ciclo de vida del software: análisis, diseño, codificación, pruebas y mantenimiento.
- Proporcionan un entorno integrado de trabajo con editores gráficos, repositorios de datos y generadores de documentación.
- Facilitan la estandarización del proceso de desarrollo.
- Permiten la reutilización de componentes y modelos.
- Mejoran la calidad del software producido al detectar errores en fases tempranas del desarrollo.

Las herramientas CASE pueden clasificarse en función de las fases del ciclo de vida que cubren, de su nivel de integración y de la funcionalidad que ofrecen. Surgieron a finales de la década de 1980 y principios de los 90 como respuesta a la creciente complejidad de los sistemas de información.

> **Dato de examen**: CASE = **Computer Aided Software Engineering**. No confundir con "Computer Aid Software Enable" ni variantes similares. Las cuatro palabras exactas son Computer, Aided, Software, Engineering.

### 13.2 Clasificación de herramientas CASE

La clasificación más habitual de las herramientas CASE se basa en las **fases del ciclo de vida del desarrollo de sistemas** que cubren. Se distinguen tres categorías principales:

| Categoría | Nombre completo | Fases que cubre | Funciones principales |
|---|---|---|---|
| **U-CASE** | Upper CASE | Planificación, análisis de requisitos, estrategia | Diagramas UML, modelado de procesos, planificación del proyecto |
| **M-CASE** | Middle CASE | Análisis y diseño de la aplicación | Automatización de tareas de análisis, diseño de arquitectura, modelado de datos |
| **L-CASE** | Lower CASE | Implementación y pruebas | Semi-automatización de la generación de código, detección de errores, depuración, pruebas, documentación automática |

**Existe también una categoría integradora:**

- **I-CASE** (Integrated CASE): herramientas que abarcan **todas las fases** del ciclo de vida, desde la planificación hasta las pruebas. También se conocen como *CASE workbench*.

> **Dato de examen**: La clasificación estándar por fases del ciclo de vida incluye **Upper CASE (U-CASE)**, **Middle CASE (M-CASE)** y **Lower CASE (L-CASE)**. La categoría **Higher CASE (H-CASE) no existe** en la clasificación estándar y es un distractor habitual en preguntas de examen.

```mermaid
---
title: Clasificación de herramientas CASE por fase del ciclo de vida
---
flowchart LR
    UCASE["U-CASE\n(Upper CASE)"]
    MCASE["M-CASE\n(Middle CASE)"]
...
- **Upper CASE (U-CASE)**: ayuda en las fases de **planificación, análisis de requisitos y estrategia** del desarrollo, utilizando entre otros diagramas UML y herramientas de gestión de requisitos.
- **Middle CASE (M-CASE)**: automatiza tareas en el **análisis y diseño** de la aplicación, como el modelado de datos, la generación de esquemas y la validación de modelos.
- **Lower CASE (L-CASE)**: semi-automatiza la **generación de código**, crea programas de detección de errores, soporta la depuración de programas y pruebas. Además, automatiza la documentación completa de la aplicación.

### 13.3 Herramientas CASE destacadas

A continuación se recogen las herramientas CASE y de gestión de proyectos que aparecen en el banco de preguntas del examen:

#### Visual Paradigm (VP-UML)

**Visual Paradigm** es una herramienta CASE profesional que soporta los estándares **UML 2**, **SysML** y **BPMN** del Object Management Group (OMG).

**Características principales:**

- **Ingeniería inversa** (*reverse engineering*): genera diagramas UML a partir de código fuente existente (Java, C++, C#, PHP, Python, Ada, entre otros).
- **Ingeniería de ida y vuelta** (*round-trip engineering*): mantiene sincronizados el modelo UML y el código fuente, permitiendo hacer cambios en uno y reflejarlos en el otro de forma bidireccional.
- Generación de código a partir de diagramas de clases UML.
- Generación de informes y documentación del proyecto.
- Soporte para múltiples lenguajes: Java, C++, CORBA IDL, PHP, XML Schema, Ada y Python.

> **Dato de examen**: Visual Paradigm es la herramienta que trabaja con **ingeniería inversa** de diagramas de código y proporciona la **ingeniería de ida y vuelta** para varios lenguajes de programación.

#### Modelio

**Modelio** es una herramienta de modelado **open source** (licencia GPLv3, liberada el 5 de octubre de 2011) desarrollada por Modeliosoft (París, Francia).

**Características principales:**

- Soporta los estándares **UML2** y **BPMN** de forma integrada en una única herramienta.
- Soporte para **TOGAF**, **SysML** y **SoaML** mediante extensiones (módulos).
- Importación/exportación **XMI** para intercambio de modelos UML2 entre herramientas.
- Sistema de extensiones modular: permite añadir soporte para diferentes lenguajes, metodologías o técnicas de modelado.
- Generación de código y documentación.

> **Dato de examen**: Modelio es la herramienta **open source** de UML que soporta los estándares **UML2 y BPMN**.

#### Poseidon for UML

**Poseidon for UML** es una aplicación de diagramado UML desarrollada por **Gentleware AG**, derivada del proyecto open source **ArgoUML**.

**Características principales:**

- Escrita íntegramente en **Java**, lo que la hace independiente de plataforma.
- Publicada bajo la **licencia BSD** (en su Community Edition).
- Soporta modelado UML completo, incluyendo diagramas de clases, secuencia, casos de uso, estados y actividades.
- Generación de código Java (forward engineering).
- Disponible en varias ediciones: Community (gratuita), Standard, Professional y Embedded.

> **Dato de examen**: Poseidon for UML es la aplicación de diagramado UML **escrita en Java** y publicada bajo **licencia BSD**.

#### Microsoft Project (MSP)

**Microsoft Project (MSP)** es un software de **administración de proyectos** diseñado, desarrollado y comercializado por Microsoft.

**Características principales:**

- Asiste a los administradores de proyectos en la elaboración de planes, asignación de recursos a tareas, seguimiento del progreso, gestión del presupuesto y análisis de cargas de trabajo.
- Permite crear diagramas de Gantt, redes PERT y calendarios de proyecto.
- Integración con el ecosistema Microsoft (SharePoint, Teams, Power BI).
- Permite definir dependencias entre tareas, hitos y rutas críticas.

> **Dato de examen**: MSP (Microsoft Project) es el software de **administración de proyectos** de Microsoft. No es una herramienta de modelado UML ni una herramienta CASE propiamente dicha.

| Herramienta | Tipo | Lenguaje/Licencia | Característica distintiva |
|---|---|---|---|
| **Visual Paradigm** | CASE (UML) | Comercial | Ingeniería inversa y round-trip engineering |
| **Modelio** | CASE (UML) | Open source (GPLv3) | Soporte integrado UML2 + BPMN |
| **Poseidon** | CASE (UML) | Java / BSD | Derivada de ArgoUML, multiplataforma |
| **MSP** | Gestión de proyectos | Comercial (Microsoft) | Planificación, Gantt, recursos, presupuestos |

### 13.4 Frameworks de desarrollo de software

Un **framework de desarrollo de software** (también llamado **entorno o marco de trabajo**) es un conjunto de herramientas, modelos y métodos que asiste al proceso de **desarrollo de software** (no de hardware). Es una filosofía de desarrollo que define la estructura, las prácticas y las pautas para organizar el trabajo de los equipos de desarrollo.

**Características de los frameworks:**

- Son herramientas, modelos y métodos para asistir al proceso de desarrollo de **software**.
- Es una filosofía de desarrollo que se suele vincular a buenas prácticas de ingeniería.
- Se le llama también **entorno o marco de trabajo**.
- Son a menudo vinculados a metodologías de gestión de proyectos y estándares de calidad.

> **Dato de examen**: La afirmación "son herramientas, modelos y métodos para asistir al proceso de desarrollo **hardware**" es **incorrecta**. Los frameworks asisten al desarrollo de **software**, no de hardware.

Los frameworks se clasifican según la naturaleza de su proceso en dos grandes grupos:

#### Frameworks iterativos

Los frameworks iterativos se caracterizan por ciclos repetitivos de desarrollo, donde el producto evoluciona a través de sucesivas iteraciones que refinan y amplían la funcionalidad.

| Framework | Descripción |
|---|---|
| **Incremental** | El software se construye en una serie de incrementos; cada incremento añade funcionalidad al producto final. Las etapas se repiten, y en cada ciclo se entrega un módulo funcional adicional. |
| **Espiral** | Propuesto por Barry Boehm en 1986. Combina elementos del modelo secuencial con la iteración. Cada ciclo de la espiral pasa por las fases de planificación, análisis de riesgos, desarrollo y evaluación. Pone especial énfasis en la **evaluación de riesgos** en cada iteración. |
| **Prototipado** | Se construye un prototipo inicial para que el cliente evalúe el producto en fases tempranas. El prototipo evoluciona en cada iteración hasta alcanzar los requisitos completos. Facilita la validación temprana con el usuario. |

#### Frameworks secuenciales

| Framework | Descripción |
|---|---|
| **Modelo en cascada (Waterfall)** | Framework secuencial y lineal en el que cada fase del desarrollo (requisitos, diseño, implementación, verificación, mantenimiento) debe completarse antes de iniciar la siguiente. No permite volver atrás de forma natural. Es el modelo **no iterativo** por excelencia. |

> **Dato de examen**: El **modelo en cascada** (Waterfall) **no es un framework iterativo**; es un framework **secuencial** donde las fases se ejecutan de forma lineal y no se repiten. Los frameworks iterativos incluyen el modelo incremental, el modelo en espiral y el prototipado.

```mermaid
---
title: Modelo en Cascada (Waterfall) — Secuencial
---
flowchart TB
...
```mermaid
---
title: Modelo en Espiral (Boehm, 1986) — Iterativo
---
flowchart TB
